...
public abstract class SinkOrSourceBuilderBase<BuilderT> {
    ...
    public SinkOrSourceBuilderBase(String prefix) {
        prefixes = new String[]{PropertiesUtils.NO_PREFIX, prefix, Constants.NATS_PREFIX + prefix};
    }
    ...
    protected void baseProperties(Properties properties) {
        List<String> subjects = PropertiesUtils.getPropertyAsList(properties, Constants.SUBJECTS, prefixes);
        if (!subjects.isEmpty()) {
            subjects(subjects);
        }

        long l = PropertiesUtils.getLongProperty(properties, Constants.STARTUP_JITTER_MIN, -1, prefixes);
        if (l != -1) {
            minConnectionJitter(l);
        }

        l = PropertiesUtils.getLongProperty(properties, Constants.STARTUP_JITTER_MAX, -1, prefixes);
        if (l != -1) {
